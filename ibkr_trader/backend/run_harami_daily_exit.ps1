# run_harami_daily_exit.ps1 -- wrapper for the 3:45pm ET weekday harami
# live-exit task (harami_daily_trader.py --mode exit). Runs BEFORE the
# close since a Market-on-Close order must be submitted before a
# same-day IBKR cutoff to participate in the closing auction.
#
# Same stdout/stderr capture discipline as run_butterfly_entry.ps1 /
# run_harami_daily_entry.ps1 (System.Diagnostics.Process, NOT
...
# a silent failure here is not acceptable -- and specifically here, a
# missed exit means a position rides past its planned 5-day hold
# unmanaged until the next exit run finds it the following day.
#
# Usage (Scheduled Task action): powershell -File run_harami_daily_exit.ps1
...
$OutLog = Join-Path $CrashDir "harami_exit_$($stamp)_stdout.log"
$ErrLog = Join-Path $CrashDir "harami_exit_$($stamp)_stderr.log"

$psi = New-Object System.Diagnostics.ProcessStartInfo
$psi.FileName = $Python
$psi.Arguments = "`"$Script`" --mode exit"
...
if ($code -ne 0) {
    ...
    try {
        ...
        $text = "&#x1F6A8; harami_daily_trader --mode exit FAILED (exit $code) -- REAL MONEY task, check now.`n`nLast stdout:`n$tail"
    ...
    $entryJson = @{
        ...
        actor     = "trader"
        category  = "harami_live_exit_task_failed"
        summary   = "harami_daily_trader --mode exit task exited $code. Full logs: $OutLog / $ErrLog"
        rationale = "Captured via run_harami_daily_exit.ps1 wrapper."
